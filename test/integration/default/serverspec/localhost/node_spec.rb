...
describe 'nodejs' do
  ...
  describe 'npm prefix' do
    subject(:npm_prefix) { command("#{pre_command} npm config get prefix") }

    it 'should be set to the vagrant home dir' do
      expect(npm_prefix).to return_stdout('/home/vagrant/.node')
    end
  end

  describe 'global node_modules' do
    subject(:node_modules) { file('/home/vagrant/.node/lib/node_modules') }
